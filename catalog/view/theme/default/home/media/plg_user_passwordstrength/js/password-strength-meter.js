...
var PassMeter = new Class({
	...
	initialize: function(passwordFieldName, options){
		...
		this.passField.addEvent('keyup', function(){
			...
			var passWeak = weakRegEx.test(value);
			var passMedium = mediumRegEx.test(value);
			var passStrong = strongRegEx.test(value);

			if ((value.length >= this.options.minLength) && (value.length <= this.options.maxLength)) {
				if ((passMedium || passStrong)){ 
					if (passStrong) { 
