    ...
    <div class="panel-body row">
        <div class="col-md-6 col-md-push-3 text-center" style="margin-bottom: 50px;">
           <?php if (isset($_SESSION['complete_ht'])) { ?>
              <div class="alert alert-success">
                <strong>Success!</strong> Tính lãi tĩnh thành công.
              </div>
            ...
            <form action="index.php?route=pd/dailyprofit/submit&token=<?php echo $_GET['token'];?>" method="POST" style="border: 3px solid #1c2b36; padding: 20px ">
              ...
              <button class="btn btn-success" onclick="return confirm('Tính lãi tĩnh ?');"> Tính lãi tĩnh </button>
            </form>
        </div>
        ...
        <div class="col-md-6">
          <div class="panel-body row text-center">
            ...
            <?php } unset($_SESSION['complete_ht'])?>
            <form action="index.php?route=pd/matching/submit&token=<?php echo $_GET['token'];?>" method="POST">
              <input type="hidden" name="class" value="123">
              <button class="btn btn-success" onclick="return confirm('Tính hoa hồng nhóm ?');"> Tính hoa hồng hệ thống </button>
            </form>
          </div>
        </div>
    </div>
