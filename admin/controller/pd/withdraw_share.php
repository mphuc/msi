<?php
class ControllerPdWithdrawshare extends Controller {
	public function index() {
		...
		$ts_history = $this -> model_pd_registercustom -> get_count_stock();
		$data['self'] =  $this;
		...
		$pagination -> url = $this -> url -> link('pd/withdraw_share', 'page={page}&token='.$this->session->data['token'].'', 'SSL');
		$data['code'] =  $this-> model_pd_registercustom->getTransctionHistory_stock($limit, $start);
		...
		$this->response->setOutput($this->load->view('pd/withdraw_share.tpl', $data));
	}
	...
	public function payment()
	{
		...
		{
			$this->load->model('pd/registercustom');
			$this -> model_pd_registercustom -> update_status_payment_stock($this -> request -> get['id']);
			$get_count_stock_id = $this -> model_pd_registercustom -> get_count_stock_id($this -> request -> get['id']);

			$this -> model_pd_registercustom -> update_m_Wallet_add_sub($get_count_stock_id['amount']*0.6, $get_count_stock_id['customer_id'], true);
			$this -> model_pd_registercustom -> update_reshare_Wallet($get_count_stock_id['amount']*0.4, $get_count_stock_id['customer_id'], true);
				
			$this -> response -> redirect($this -> url -> link('pd/withdraw_share&token='.$_GET['token'].''));
		}
	}
